...
contract Event is IEvent, Ownable, ReentrancyGuard, ContractMetadataLogic, ERC721URIStorage {
    ...
    function buy(uint256 amount, address buyer) external payable override onlyOwner returns (bool) {
        ...
        require(
            (amount <= _maxAllowedTicketsPerUser) &&
                (ERC721.balanceOf(buyer) + amount <= _maxAllowedTicketsPerUser),
    ...
    function close() external override onlyOwner nonReentrant returns (bool) {
        ...
        (bool success, ) = creator.call{value: amount.sub(commission)}("");
        require(success, "Failed to transfer");

        (bool successCom, ) = _owner.call{value: commission}("");
